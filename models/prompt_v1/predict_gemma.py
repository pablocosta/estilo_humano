...
model, tokenizer = FastLanguageModel.from_pretrained(
        model_name = "./gemma_finetuned/checkpoint-342/", # YOUR MODEL YOU USED FOR TRAINING
        max_seq_length = 300,
...
texts = [tokenizer.apply_chat_template(ele, tokenize = False, add_generation_prompt = True).removeprefix('<bos>') for ele in reasoning_valid["conversations"]]


generated = []
for ele in tqdm(texts):
    generated_ids = model.generate(**tokenizer(ele, return_tensors = "pt").to("cuda"), temperature = 1.0, top_p = 0.95, top_k = 64, max_new_tokens = 1024)

    generated.append(tokenizer.batch_decode(generated_ids, skip_special_tokens=True, clean_up_tokenization_spaces=True)[0].strip())

def write_file(data):
    
    with open("./preds_gemma.txt", "w+", encoding="utf-8") as outfile:
        outfile.write("\n".join(data))
